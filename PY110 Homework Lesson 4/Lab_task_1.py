...
BOOKS_FILE = "books.md"
BOOK_REGEX = r'####\s+(?P<position>\d+)\.\s\[(?P<book>.+?)\]\((?P<book_url>.+?)\)' \
             r'\s\w{2}\s+(?P<author>.+?)\s+\((?P<recommended>\d{1,3}\.\d+%)\s\w{11}\)' \
             r'\s+\S\S\S\((?P<cover_url>.+?)\)\s+\"(?P<description>.+?)\"'  # TODO записать ругулярное выражения для поиска книги
